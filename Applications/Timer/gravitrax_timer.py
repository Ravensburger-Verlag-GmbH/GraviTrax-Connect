...
async def notification_callback(bridge: gb.Bridge, **signal):
    ...
    if signal.get("Header"):  # Check if Notification was a signal
        ...
        if stone == gv.STONE_FINISH and color == FIN_COLOR:
            if start_times:
                gb.log_print(f"Time between Start and Finish {time.time()-start_times.pop(0)}")
...
def on_press(key):
    ...
    if key == Key.esc:
        gb.log_print("Stop Signal received: Disconnecting Bridge", bridge=b)
...
async def main():
    ...
    if await b.connect(try_reconnect=True, dc_callback=disconnect_callback):
        ...
        gb.log_print("Could find Bridge to connect to")
    await finished.wait()
