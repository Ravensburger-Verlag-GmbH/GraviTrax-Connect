...
The gravitraxconnect library uses Logger for status Messages. log_print can be used to
make prints using this Logger
"""
...
async def main():
    ...
    gb.log_print("A simple log print")
    # Passing multiple Arguments
    gb.log_print("Passing ", 3, " Arguments")
    try:
        if await bridge.connect():
            # Passing the Bridge to print the address
            gb.log_print("<--- This is the MAC-address of the Bridge", bridge=bridge)
        else:
            gb.log_print("Could not connect to Bridge")
